...
namespace crem
{
	class AppWindow : public TCrem::TAppWindow<sf::RenderWindow>
	{
	public:
	/** \brief Initialize the screen with custom parameters
	 * Look at sfml_SetVideoMode to have informations on this function (the only differences are that there is a verification and a try to initialise video subsystem before if necessary, and that some logs are generated if there are errors)
	 * \param width const uint16_t
	 * \param height const uint16_t
	 * \param bpp const uint8_t bits per pixel
	 * \param flags const uint32_t
	 */
		void init(const uint16_t width, const uint16_t height, const uint8_t bpp);
		void init(void);
